...
const searchSchema = z.object({ mode: z.enum(["login", "register", "forgot", "verify"]).optional() });
...
function AuthPage() {
  ...
  useEffect(() => {
    if (!authLoading && user) {
      if (user.email_confirmed_at) {
        ...
        if (mode !== "verify") {
          navigate({ to: "/auth", search: { mode: "verify" }, replace: true });
        }
      }
    }
  ...
  const handleResendEmail = async () => {
    ...
    if (!targetEmail) {
      toast.error("Alamat email tidak ditemukan. Silakan masuk kembali.");
      return;
    }
    setLoading(true);
    try {
  ...
  const handleSignOutOrBack = async () => {
    if (user) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "register") {
        ...
        navigate({ to: "/auth", search: { mode: "verify" } });
      } else if (mode === "forgot") {
  ...
  const formSub = mode === "register"
    ? "Tempat aman untuk tumbuh dan memahami diri."
    : mode === "forgot"
      ? "Kami akan kirim link reset ke email kamu."
      : mode === "verify"
        ? "Langkah terakhir sebelum masuk ke aplikasi."
            ...
            <div className="rounded-3xl bg-card p-8 shadow-elevated ring-1 ring-border animate-scale-in">
              ...
              {mode === "verify" ? (
                <div className="mt-6 space-y-6">
                  <div className="rounded-2xl bg-[#F4F7F5] p-5 text-center border border-[#6E8C71]/20">
                    <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#6E8C71]/15 text-[#6E8C71]">
                    ...
                    <p className="mt-4 text-sm text-foreground leading-relaxed font-medium">
                      Kami telah mengirim email verifikasi. Silakan cek inbox atau spam Anda.
                    </p>
                    {(email || user?.email) && (
                      <p className="mt-2 text-xs text-muted-foreground font-mono bg-background/60 py-1 px-2.5 rounded-lg inline-block select-all border border-border/30">
                        {email || user?.email}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full rounded-full bg-accent py-3.5 text-sm font-semibold text-accent-foreground shadow-peach transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-60 flex items-center justify-center gap-2 cursor-pointer"
                    >
                      {loading ? "Memproses…" : resendCooldown > 0 ? `Kirim Ulang (${resendCooldown}s)` : "Kirim Ulang Email Verifikasi"}
                    </button>

                    <button
                      type="button"
                      onClick={handleSignOutOrBack}
                      className="flex w-full items-center justify-center gap-2 rounded-full border border-border bg-background py-3 text-sm font-semibold text-foreground transition-all hover:bg-cream-deep cursor-pointer"
                    >
                ...
                <>
                  <form onSubmit={submit} className="mt-6 space-y-3">
                    ...
                    <button
                      disabled={loading}
                      type="submit"
                      className="w-full rounded-full bg-accent py-3.5 text-sm font-semibold text-accent-foreground shadow-peach transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-60 cursor-pointer"
                    >
                  ...
                  {mode !== "forgot" && (
                    <>
                      ...
                      <button
                        onClick={google}
                        disabled={loading}
                        className="flex w-full items-center justify-center gap-3 rounded-full border border-border bg-background py-3 text-sm font-semibold text-foreground transition-all hover:bg-cream-deep disabled:opacity-60 cursor-pointer"
                      >
                        {/* Google G logo */}
