    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      >
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="font-display text-xl sm:text-3xl font-semibold">Emergency Calm Mode</h1>
          ...
          <button
            ...
            className="flex items-center gap-2 self-start sm:self-center rounded-full bg-card/65 hover:bg-card/90 border border-border/40 px-3.5 py-1.5 text-[10px] sm:px-4 sm:py-2 sm:text-xs font-semibold text-foreground backdrop-blur-md transition-all duration-200 shadow-sm active:scale-95 cursor-pointer"
          >
            <span className={`text-base ${isCanonPlaying ? "animate-pulse" : ""}`}>🎻</span>
            <span>Canon in D: {isCanonPlaying ? "Diputar" : "Dijeda"}</span>
      ...
      <div className="grid gap-2.5 sm:gap-3 sm:grid-cols-2">
        {tools.map((t) => (
          <button
            ...
            className={`rounded-3xl p-4 sm:p-5 text-left ring-1 transition-all duration-250 hover:-translate-y-0.5 card-lift ${
              tool === t.k
                ? "ring-primary/40 shadow-soft bg-card"
                : "bg-card ring-border/60 shadow-card"
            }`}
          >
            <p className="text-xl sm:text-2xl">{t.icon}</p>
            <h3 className="mt-2 font-display text-base sm:text-lg font-semibold text-foreground">{t.title}</h3>
            <p className="mt-0.5 text-[11px] sm:text-xs text-muted-foreground">{t.desc}</p>
            {tool === t.k && (
              <div
