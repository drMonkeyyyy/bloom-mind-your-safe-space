    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative flex items-center justify-between gap-3">
          <div>
            <h1 className="font-display text-xl sm:text-3xl font-semibold">Gratitude Journal</h1>
            <p className="mt-0.5 text-[11px] sm:text-sm text-muted-foreground">Tiga hal kecil yang kamu syukuri hari ini.</p>
          </div>
          {monthCount > 0 && (
            <span className="shrink-0 rounded-full bg-white/60 backdrop-blur-sm px-2 py-1 text-[10px] sm:px-3 sm:py-1.5 sm:text-xs font-semibold text-amber-700 shadow-sm" style={{ animation: "streak-pulse 2.4s ease-in-out infinite" }}>
              🙏 {monthCount}×
            </span>
          )}
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 space-y-4">
          <section className="rounded-3xl bg-card p-4.5 sm:p-6 ring-1 ring-border/60 shadow-card space-y-3.5 sm:space-y-4">
            <div className="flex items-center gap-3">
              <div
                className="grid h-9 w-9 place-items-center rounded-2xl text-lg shadow-sm animate-pulse-slow"
                style={{ background: "oklch(0.96 0.04 75)" }}
              >
                🙏
              </div>
              <p className="text-sm sm:text-base font-semibold text-foreground">Aku bersyukur untuk…</p>
            </div>
            ...
            {([
              ...
              <div key={key} className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-base font-bold select-none" style={{ color: "oklch(0.82 0.14 75 / 0.5)" }}>
                  {num}.
                </span>
                <input
                  value={g[key]}
                  ...
                  className="w-full rounded-2xl border border-border bg-background py-2.5 pl-10 pr-4 text-xs sm:text-sm transition-all duration-200"
            ...
            <div className="border-t border-border/60 pt-3.5 space-y-3">
              <div>
                <label className="mb-1 flex items-center gap-1.5 text-xs sm:text-sm font-semibold" htmlFor="best-moment">
              ...
              <div>
                <label className="mb-1 flex items-center gap-1.5 text-xs sm:text-sm font-semibold" htmlFor="lesson">
                  💡 Pelajaran hari ini
                </label>
                <input
                  id="lesson"
      ...
      {/* ── HISTORY ─────────────────────────────────────────────── */}
      {oldGratitudesCount && oldGratitudesCount > 0 && !cleanupSnoozed ? (
        <div className="flex items-center justify-between gap-3 rounded-2xl bg-amber-50/70 border border-amber-100/50 p-3.5 text-xs text-amber-900 animate-slide-up mb-4">
          <div className="flex items-center gap-2">
            ...
            <p className="leading-relaxed">
              Terdapat catatan syukur yang sudah berjalan lebih dari 3 bulan. Bersihkan riwayat lama untuk menghemat ruang?
              {warnedAt && (() => {
                const diff = Date.now() - warnedAt;
                const remainingDays = 30 - Math.floor(diff / (24 * 60 * 60 * 1000));
      ...
      >
        <div className="space-y-4">
          <p className="text-xs text-muted-foreground leading-normal">
            Catatan syukur yang lebih lama dari 1 bulan (30 hari) akan dihapus secara permanen dari database. Catatan 1 bulan terakhir tetap disimpan di aplikasi.
          </p>
          <div className="rounded-2xl bg-amber-50 border border-amber-100 p-3.5 flex gap-3 text-amber-950">
            ...
            <div className="text-xs leading-relaxed">
              ...
              <p>Apakah Anda ingin menyimpan seluruh catatan syukur yang akan dihapus (di atas 1 bulan) sebagai **PDF** atau mendownload file **JSON** di laptop Anda sebelum dihapus?</p>
            </div>
          </div>
