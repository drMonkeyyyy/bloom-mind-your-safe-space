...
const primaryNav = [
  ...
  { to: "/app/community", label: "Komunitas Anonim", icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" },
...
const wellnessNav = [
  { to: "/app/program", label: "Program Pemulihan", icon: "M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" },
  { to: "/app/calm-check", label: "Cek Kesehatan Mental", icon: "M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" },
...
const bottomNav = [
  { to: "/app", label: "Beranda", icon: "M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z", exact: true },
  { to: "/app/community", label: "Komunitas", icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" },
  { to: "/app/mood", label: "Mood", icon: "M12 20h9M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z" },
];

const moreNavItems = [
  { to: "/app/program", label: "Program Pemulihan", icon: "M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5", color: "bg-amber-50 text-amber-600" },
  { to: "/app/community", label: "Dinding Dukungan Anonim", icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75", color: "bg-teal-50 text-teal-600" },
...
// Extracted StreakBuddy, AffirmationWidget, and BloomGarden to separate files.
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r border-border/40 bg-card/60 backdrop-blur-2xl lg:flex shadow-[4px_0_30px_rgba(0,0,0,0.015)]">
        ...
        <nav className="relative flex-1 overflow-y-auto space-y-5 px-3 py-2">
          ...
          {/* Place widgets inside scrollable nav */}
          <div className="pt-4 border-t border-border/20 space-y-3.5">
            <BloomGarden userId={user?.id} />
            <AffirmationWidget />
      ...
      {/* ── MAIN CONTENT ─────────────────────────────────── */}
      <main className={`lg:pl-64 lg:pb-12 ${isChatRoom ? "pb-0" : "pb-16"}`}>
        <div className="mx-auto max-w-5xl px-3.5 py-4 sm:px-6 sm:py-8">
          ...
          <div className="glass-strong grid grid-cols-5 items-end pb-safe">
            ...
            {bottomNav.slice(0, 2).map((n) => {
              ...
              return (
                <Link
                  key={n.to}
                  to={n.to}
                  className={`flex flex-col items-center gap-0.5 py-1.5 text-[9px] tracking-tight font-semibold transition-all duration-200 ${active ? "text-primary font-bold" : "text-muted-foreground hover:text-foreground"}`}
                >
                  <span className={`relative flex h-6 w-6 items-center justify-center rounded-lg transition-all duration-200 ${active ? "bg-primary-soft scale-105 shadow-xs" : "hover:bg-cream-deep"}`}>
            ...
            <div className="flex flex-col items-center pb-0.5">
              <div className="relative">
                <Link
                  to="/app/chat"
                  className="group flex h-11 w-11 -translate-y-2 items-center justify-center rounded-full bg-gradient-to-br from-primary to-accent shadow-md transition-all duration-300 hover:scale-105 active:scale-95"
                  aria-label="Buka Teman Curhat"
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5" aria-hidden="true">
            ...
            <button
              ...
              className={`flex flex-col items-center gap-0.5 py-1.5 text-[9px] tracking-tight font-semibold transition-all duration-200 ${moreOpen ? "text-primary font-bold" : "text-muted-foreground hover:text-foreground"}`}
              aria-label="Lebih banyak menu"
              aria-expanded={moreOpen}
            >
              <span className={`relative flex h-6 w-6 items-center justify-center rounded-lg transition-all duration-200 ${moreOpen ? "bg-primary-soft scale-105 shadow-xs" : "hover:bg-cream-deep"}`}>
